...
import { getStatusColor, getStatusSymbol, getProtectionSymbol } from './StatusIndicator';
...
function ServiceRow({ service, isSelected, index }: ServiceRowProps) {
  ...
  const bgColor = isSelected ? '#2563eb' : (index % 2 === 0 ? '#1f2937' : '#111827');
  const fgColor = isSelected ? '#ffffff' : '#e5e7eb';
  
  // Type indicator
  const typeIndicator = service.type === 'LaunchDaemon' ? 'D' 
    : service.type === 'LaunchAgent' ? 'A' 
    : 'E';
  
  const domainIndicator = service.domain === 'system' ? 'sys'
    : service.domain === 'user' ? 'usr'
    : 'gui';
  ...
  return (
    ...
    >
      ...
      {/* Type and Domain */}
      <box width={8}>
        <text fg="#6b7280">{typeIndicator}/{domainIndicator}</text>
      </box>
...
export function ServiceList() {
  ...
  const startIndex = Math.max(0, state.selectedIndex - Math.floor(listHeight / 2));
        ...
        <box flexGrow={1}>
          <text fg="#9ca3af">
            ...
            {state.sort.field === 'label' && (
              <span fg="#60a5fa"> {state.sort.direction === 'asc' ? '▲' : '▼'}</span>
            )}
          </text>
        </box>
        <box width={8} justifyContent="flex-end">
