...
function AppContent() {
  ...
  useKeyboard((key) => {
    ...
    // Page up/down
    if (key.name === 'pageup') {
      dispatch({ type: 'SELECT_INDEX', payload: Math.max(0, state.selectedIndex - 10) });
      return;
    }
    if (key.name === 'pagedown') {
      dispatch({ type: 'SELECT_INDEX', payload: Math.min(filteredServices.length - 1, state.selectedIndex + 10) });
      return;
    }
    ...
    if (key.name === 'a' && !key.shift) {
      dispatch({ type: 'SET_FILTER', payload: { showAppleServices: !state.filter.showAppleServices } });
      return;
    }
    if (key.name === 'p') {
      dispatch({ type: 'SET_FILTER', payload: { showProtected: !state.filter.showProtected } });
      return;
    }
