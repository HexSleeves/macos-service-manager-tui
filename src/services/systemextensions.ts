...
import type { SystemExtension, ServiceStatus, ProtectionStatus } from '../types';
...
export function parseSystemExtensionsList(output: string): SystemExtension[] {
  ...
  for (const line of lines) {
    ...
    // Extension line
    const extMatch = line.match(/^\s*(enabled|disabled)?\s*(active|inactive|terminated|waiting)?\s*(\w{10})?\s+([\w.]+)(?:\s+\[([^\]]+)\])?/);
    if (extMatch) {
...
function mapExtensionState(state?: string): SystemExtension['state'] {
  switch (state) {
    case 'active': return 'activated_enabled';
    case 'waiting': return 'activated_waiting';
    case 'terminated': return 'terminated';
    case 'inactive': return 'uninstalled';
 ...
 */
export async function uninstallSystemExtension(bundleId: string): Promise<{ success: boolean; message: string; error?: string }> {
 ...
 */
export async function resetSystemExtensions(): Promise<{ success: boolean; message: string; error?: string }> {
